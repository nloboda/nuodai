...
{
	char* special_block = this->block_mapper->get_block();
	std::unique_ptr<char> new_special_block = std::unique_ptr<char>(new char[32]);
	this->crypto_layer->write_encrypted(special_block, new_special_block.get(), this->special_block_iv.get());
	return new_special_block;
